...
function getTargetBytes(inputEl, toggleBtn) {
...
class ToolCard {
  ...
  bindEvents() {
    ...
    // Size unit toggles
    this.el.querySelectorAll(".size-unit-toggle").forEach((btn) => {
      btn.addEventListener("click", () => {
        const current = btn.dataset.unit;
        btn.dataset.unit = current === "KB" ? "MB" : "KB";
  ...
  async handleFiles(browserFiles, dragPaths = {}) {
    ...
    if (this.pendingFiles.length > 0) {
      ...
      const firstFile = this.pendingFiles[0];
...
function updateImageSmartSuffix() {
  ...
  const quality = parseInt(document.querySelector("#tool-image-converter .quality-slider")?.value) || 85;
  const scale = document.getElementById("ic-scale")?.value || "100";
  const stripMeta = document.getElementById("ic-strip-meta")?.checked;
  const mode = imageCard?.getCompressionMode?.() || "quality";
...
const imageCard = new ToolCard({
  ...
  getPayload: (card, file) => {
    ...
    const payload = {
      ...
      quality: 85,
      stripMeta: document.getElementById("ic-strip-meta").checked,
      resize: resize,
      targetBytes: 0,
...
document.getElementById("ic-strip-meta")?.addEventListener("change", () => {
  updateImageSmartSuffix();
});
